...
def multi_shotGentrainer(data_loader_area, data_loader_shoot, encoder, decoder, criterion, encoder_optimizer,decoder_optimizer, config, device="cpu"):
    encode_length = config['encode_length'] - 1  # use the first 3 strokes to the encoder
    record_loss = {
        'total': [],
        'shot': [],
    ...
    with torch.no_grad():
        ...
        for sample_id in range(samples):
            ...
            for seq_idx in range(encode_length, given_seq['rally_length']-1):
                ...
                cov = torch.zeros(2, 2).cpu()#output_xy.device)
                cov[0, 0]= sx * sx
                cov[0, 1]= corr * sx * sy
                cov[1, 0]= corr * sx * sy
                cov[1, 1]= sy * sy
